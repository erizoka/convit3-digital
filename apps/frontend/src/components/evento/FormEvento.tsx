...
export default function FormEvento() {
    ...
    const permiteProximoPasso: boolean[] = [
        ...
        && !!evento.imagem && !!evento.imagemBackground
    ]
        ...
        <div>
            ...
            >
                <div className="flex flex-col gap-5">
                    <CampoEntrada label="Identificador" value={Alias.formatar(evento.alias ?? "")}
                        descricao="Identificador único e exclusivo para o evento (usado na URL)"
                        onChange={(e) => alterarEvento({
                            ...evento,
                            alias: Alias.formatar(e.target.value)
                        })}
                        erro={aliasValido ? "" : "Identificador já foi usado em outro evento!"}
                    />
                    <CampoEntrada label="Nome" value={evento.nome ?? ""}
